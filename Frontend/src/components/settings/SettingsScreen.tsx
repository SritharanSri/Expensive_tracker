...
import {
  ...
  BrainCircuit, Infinity as InfinityIcon, Lock
} from "lucide-react";
...
export function SettingsScreen() {
  ...
  return (
    ...
    >
      ...
      <BottomSheet open={showSignOut} onClose={() => setShowSignOut(false)} isDark={isDark}>
        <div className="px-5 pb-8 pt-2">
          <div className="flex flex-col items-center gap-4 py-4 text-center">
            ...
            <div>
              <p className={cn("font-bold text-lg", isDark ? "text-white" : "text-slate-900")}>Sign Out?</p>
              <p className={cn("text-sm mt-1 text-slate-500")}>You'll need to sign in again to access your data.</p>
            </div>
          </div>
          <div className="flex gap-3 mt-4">
            <button onClick={() => setShowSignOut(false)} className="flex-1 py-4 rounded-2xl bg-slate-100 dark:bg-slate-800 font-bold text-sm">Cancel</button>
            <button onClick={() => { setShowSignOut(false); signOut(); }} className="flex-1 py-4 rounded-2xl bg-rose-500 text-white font-bold text-sm">Sign Out</button>
          </div>
        </div>
      </BottomSheet>
    </motion.div>
